...
const PLANS = [
  {
    ...
    color: "blue",
    features: [
      "До 3-х фотографий проекта",
      "Базовая информация о ЖК",
      "Стандартное размещение в каталоге",
      "Доступ к статистике лидов",
      "Email поддержка"
    ]
  },
  {
    ...
    isPopular: true,
    features: [
      "До 7-ми фотографий проекта",
      "Возможность добавить видео (YouTube/Insta)",
      "Плашка 'Verified Developer'",
  ...
  },
  {
    ...
    features: [
      ...
      "Плашки 'Популярное' и 'Проверенное'",
      "Всегда в топе рекомендаций",
      "Персональный менеджер",
      "Выгрузка отчетов в PDF/Excel",
      "Прямая интеграция с CRM"
    ]
  }
];
...
export default function SubscriptionsPage() {
  ...
  const handleUpgrade = async (projectId: number, plan: string) => {
    if (!projectId) {
      alert("Пожалуйста, сначала выберите проект");
      return;
    }
    ...
    try {
      ...
      setActivePayment({
        ...
        instructions: data.instructions?.comment || "Оплатите по реквизитам",
      });
    } catch (err) {
      console.error("Upgrade error:", err);
      setError("Ошибка оформления подписки. Попробуйте позже.");
    } finally {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      <div className="space-y-2">
        <h1 className="text-4xl font-black text-[#1E3A8A] tracking-tight">Подписки и Тарифы</h1>
        <p className="text-slate-500 font-medium max-w-2xl">
          Выберите подходящий тариф для ваших жилых комплексов, чтобы привлечь больше покупателей и получить доступ к эксклюзивным инструментам продвижения.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        {PLANS.map((plan) => (
          <div key={plan.id} className={`relative group rounded-[3rem] border border-slate-100 bg-white p-10 shadow-sm transition-all hover:shadow-2xl hover:-translate-y-2 flex flex-col ${plan.isPopular ? 'ring-4 ring-orange-500/10' : ''}`}>
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-2 uppercase italic">{plan.name}</h2>
            <div className="flex items-baseline gap-1 mb-8">
              <span className="text-4xl font-black text-slate-900">{formatUzs(plan.price)}</span>
              <span className="text-slate-400 font-bold text-sm uppercase">/ мес</span>
            </div>

            <div className="flex-1 space-y-4 mb-10">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50 pb-2">Что включено:</p>
            ...
            <div className="space-y-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Применить к проекту:</p>
                ...
                >
                  ...
                  {projects.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} {p.plan === plan.id ? '(Уже подключен)' : ''}
                    </option>
                  ))}
                </select>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="w-full max-w-xl rounded-[3rem] bg-white p-10 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-black text-slate-900 uppercase">Счёт на оплату</h3>
                <p className="text-sm font-medium text-slate-500">Заявка #{activePayment.externalRef}</p>
              </div>
            </div>

            <div className="space-y-6 bg-slate-50 p-8 rounded-[2rem] border border-slate-100 mb-8">
              <div className="flex justify-between border-b border-slate-200 pb-4">
                <span className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Тариф</span>
                <span className="font-black text-slate-900">{activePayment.plan}</span>
              </div>
              <div className="flex justify-between border-b border-slate-200 pb-4">
                <span className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Сумма к оплате</span>
                <span className="font-black text-slate-900 text-xl">{formatUzs(activePayment.amountUzs)}</span>
              </div>
              <div className="space-y-2 pt-2">
                <span className="text-slate-500 font-bold uppercase text-[10px] tracking-widest block">Инструкция</span>
